...
const Filtro = () => {
  ...
  const btnFiltro = <button className={`${classes.btnAbrirFiltro} ${menuVisible ? classes.filtroVisible : ''}`} aria-label='Abrir filtro' type='button' onClick={() => setMenuVisible(true)}><IconFiltro /></button>

  return (
    <>
      <div ref={refBtnFiltro}>
        {btnFiltro}
        {menuVisible && <section className={`${classes.filtro} ${menuVisible ? classes.filtroVisible : ''}`}>
          <ButtonFechar onClose={() => setMenuVisible(false)} className={classes.btnFechar} />
